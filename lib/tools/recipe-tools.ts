...
async function searchWebForRecipes(
  ...
  const searchQuery = `recipe ${ingredients.join(" ")} ${
    query || ""
  } (site:allrecipes.com OR site:foodnetwork.com OR site:bbcgoodfood.com OR site:seriouseats.com OR site:tasty.co OR site:simplyrecipes.com OR site:bonappetit.com OR site:epicurious.com OR site:eatingwell.com)`;
  ...
  try {
    ...
    $(".result").each((i: number, elem: any) => {
      ...
      const rawLink = $elem.find(".result__a").attr("href");
...
export const recipeSearchTool = tool({
  ...
  inputSchema: z.object({
    ...
    query: z
      ...
      .optional()
      .describe(
        "Additional search parameters like cuisine type or dietary restrictions"
      ),
    limit: z
      .number()
      .optional()
  ...
  execute: async ({ ingredients, query, limit = 10 }) => {
    try {
      const searchResults = await searchWebForRecipes(
        ingredients,
        query,
        limit * 2
      ); // Get extra to filter
...
export const recipeExtractionTool = tool({
  description:
    "Extract structured recipe data from a URL using schema.org/Recipe format or AI extraction",
  inputSchema: z.object({
  ...
  execute: async ({ url }) => {
    try {
        ...
        const recipe: Recipe = {
          ...
          servings: jsonLdRecipe.recipeYield
            ? typeof jsonLdRecipe.recipeYield === "number"
              ...
              : parseInt(jsonLdRecipe.recipeYield)
            : undefined,
          nutrition: jsonLdRecipe.nutrition
            ? {
                ...
                carbohydrates:
                  jsonLdRecipe.nutrition.carbohydrateContent || undefined,
      ...
      $('.recipe-ingredient, .ingredient, [itemprop="recipeIngredient"]').each(
        (_i: number, elem: any) => {
      ...
      $(
        '.recipe-instruction, .instruction, .direction, [itemprop="recipeInstructions"]'
